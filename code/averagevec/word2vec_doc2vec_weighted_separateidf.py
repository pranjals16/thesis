...
def makeFeatureVec(words, model, num_features,index2word_set,idf_score):
	...
	for word in words:
		if word in index2word_set:
			...
			if word in idf_score:
				temp[0:num_features]=np.multiply(model[word],counts[word]*idf_score[word]*idf_score[word])
			else:
...
def getAvgFeatureVecs(reviews, model, num_features,idf_score):
    counter = 0.
    #
    ...
    for review in reviews:
       ...
       reviewFeatureVecs[int(counter)] = makeFeatureVec(review, model, num_features,index2word_set,idf_score)
       counter = counter + 1.
...
if __name__ == '__main__':
    ...
    for word in temp:
    		...
    		i=i+1
    '''
    idf_score=cPickle.load(open('score.p', 'rb'))
    print "Creating average feature vecs for training reviews"
    trainDataVecs = getAvgFeatureVecs( getCleanReviews(train), model, num_features,idf_score)
    ...
    testDataVecs = getAvgFeatureVecs( getCleanReviews(test), model, num_features,idf_score)
