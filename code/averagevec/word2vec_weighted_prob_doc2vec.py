...
def makeFeatureVec(words, model, num_features,score):
	...
	for word in words:
		if word in index2word_set:
			...
			if word in score:
				#print score[word]
				temp=np.multiply(model[word],score[word])
			else:
				temp=model[word]
			nwords = nwords + 1.
...
def getAvgFeatureVecs(reviews, model, num_features,score):
    counter = 0.
    #
    ...
    for review in reviews:
       ...
       reviewFeatureVecs[int(counter)] = makeFeatureVec(review, model, num_features,score)
       counter = counter + 1.
...
if __name__ == '__main__':
    ...
    #
    score=cPickle.load(open('score.p', 'rb'))
    print "Creating average feature vecs for training reviews"
    trainDataVecs = getAvgFeatureVecs( getCleanReviews(train), model, num_features,score )
    #cPickle.dump(trainDataVecs, open('save_train.p', 'wb'))
    #trainDataVecs = cPickle.load(open('save_train.p', 'rb'))
    
    print "Creating average feature vecs for test reviews"
    testDataVecs = getAvgFeatureVecs( getCleanReviews(test), model, num_features,score )
