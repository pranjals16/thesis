...
def makeFeatureVec(words, model,num_features,X,counter,feature_names):
...
def getAvgFeatureVecs(reviews, model, num_features,X,feature_names):
    counter = 0.
    new_feature=X.shape[1]
    ...
    for review in reviews:
       ...
       reviewFeatureVecs[int(counter)] = makeFeatureVec(review, model,num_features,X,counter,feature_names)
       counter = counter + 1.
...
if __name__ == '__main__':
	...
	input_file = 'new_movie_reviews2.txt'
	...
	vectorizer = TfidfVectorizer(tokenizer=tokenize,use_idf=True,max_df=0.8,min_df=0.001,strip_accents='unicode')
